...
| `x` | actions menu: run, run with parameters, pause/resume schedule, repair, cancel, delete (type the name back); in an open run, each task's page; start/stop pipeline; start/terminate cluster; start/stop warehouse |
...
# open_command = "firefox --new-tab"   # URL appended; default $BROWSER or the platform opener
# copy_command = "xsel --clipboard"    # text on stdin; default clip, pbcopy, wl-copy or xclip
