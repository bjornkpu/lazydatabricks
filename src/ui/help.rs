...
fn bindings(focus: Panel, keys: &Keymap) -> Vec<(String, &'static str)> {
    ...
    let mut rows = match focus {
        ...
        Panel::Clusters => vec![
            (k(Action::Down), "next cluster"),
            (k(Action::Up), "previous cluster"),
            (k(Action::Filter), "filter by name or creator"),
            (k(Action::MineOnly), "toggle mine only"),
            (k(Action::StatusFilter), "cycle status: all, error, active"),
            (k(Action::Open), "focus the main panel"),
            (k(Action::Menu), "actions menu: start or terminate"),
            (k(Action::Refresh), "refresh clusters"),
            (k(Action::Browse), "open cluster in browser"),
            (k(Action::Copy), "copy cluster URL"),
            (k(Action::CopyTable), "copy the cluster list as text"),
        ],
        Panel::Status => vec![(k(Action::Refresh), "refresh jobs, pipelines and clusters")],
    };
    rows.extend([
